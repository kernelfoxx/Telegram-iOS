...
final class ImageBlockBoxTests: XCTestCase {
    ...
    func test_imageBox_tokenMath() {
        let block = ImageBlock(id: BlockID("img"), assetID: "x", naturalSize: Size2D(width: 100, height: 60),
                               caption: [TextRun(text: "Cap")])
        let box: CanvasBlock = ImageBlockBox(image: block, mapper: AttributedStringMapper(), width: 300)
        box.nodeStart = 5
        ...
        guard case .image(let out) = box.currentBlock() else { return XCTFail("expected image") }
    ...
    func test_currentBlocks_roundTripsMixedDocument() {
        ...
        guard case .paragraph = blocks[0], case .image = blocks[1], case .paragraph = blocks[2]
        else { return XCTFail("expected paragraph/image/paragraph") }
    }
    ...
    func test_caption_isCentered() {
        let block = ImageBlock(id: BlockID("img"), assetID: "x", naturalSize: Size2D(width: 100, height: 60),
                               caption: [TextRun(text: "Cap")])
        let box = ImageBlockBox(image: block, mapper: AttributedStringMapper(), width: 300)
    ...
    func test_emptyCaption_reservesOneLine() {
        ...
        let empty = ImageBlock(id: BlockID("img"), assetID: "x",
                               naturalSize: Size2D(width: 100, height: 60), caption: [])
        let box = ImageBlockBox(image: empty, mapper: mapper, width: 300)
    ...
    func test_emptyCaption_showsAddCaptionPlaceholder() {
        ...
        // The rect must cover the FULL reserved line (font.lineHeight × body lineHeightMultiple), matching
        // captionEmptyLineHeight — not the bare font.lineHeight — so the placeholder aligns with real text.
        let ps = mapper.styleSheet.paragraphStyle(for: .body, attributes: ParagraphAttributes(alignment: .center), list: nil)
        let mult = ps.lineHeightMultiple > 0 ? ps.lineHeightMultiple : 1
        let bodyLine = mapper.styleSheet.font(for: .body, attributes: .plain).lineHeight
        XCTAssertEqual(ph?.rect.height ?? 0, bodyLine * mult, accuracy: 0.5)

        let filled = ImageBlock(id: BlockID("img"), assetID: "x",
                                naturalSize: Size2D(width: 100, height: 60), caption: [TextRun(text: "Cap")])
        let box2 = ImageBlockBox(image: filled, mapper: mapper, width: 300)
    ...
    func test_emptyCaption_caretRectSpansTheLineHeight() {
        ...
        let v = DocumentCanvasView()
        v.imageProvider = { _ in self.solid(CGSize(width: 100, height: 60)) }
        v.setBlocks([.image(ImageBlock(id: BlockID("img"), assetID: "x",
                                       naturalSize: Size2D(width: 100, height: 60), caption: []))], width: 300)
        v.frame = CGRect(x: 0, y: 0, width: 300, height: 400); v.layoutIfNeeded()
        let imgBox = v.boxes[0] as! ImageBlockBox
        let caret = v.caretRect(for: DocumentTextPosition(imgBox.textStart))
        // The empty-caption caret spans the real line height (font.lineHeight × body lineHeightMultiple),
        ...
        XCTAssertGreaterThan(caret.height, 20.5)   // genuinely taller than the old fixed-20 fallback
    }

    func test_imageRect_respectsExplicitDisplayWidth() {
        let block = ImageBlock(id: BlockID("img"), assetID: "x", naturalSize: Size2D(width: 100, height: 50),
                               displayWidth: 60, caption: [])
        let v = DocumentCanvasView()
        v.setBlocks([.image(block)], width: 390)
        v.frame = CGRect(x: 0, y: 0, width: 390, height: 400); v.layoutIfNeeded()
        let box = v.boxes[0] as! ImageBlockBox
        XCTAssertEqual(box.imageRect().width, 60, accuracy: 0.5)   // explicit width honored, not stretched full-bleed
    }
}
#endif

...
final class MentionDateMarkersTests: XCTestCase {
    func testMentionRoundTrip() {
        ...
        XCTAssertEqual(url, "rg://user?id=12345")
    ...
    func testMentionParseRejectsNonMatching() {
        ...
        XCTAssertNil(parseMentionPeerId(fromURL: "rg://username?id=1"))
        XCTAssertNil(parseMentionPeerId(fromURL: "rg://user?id=abc"))
        XCTAssertNil(parseMentionPeerId(fromURL: "rg://user?id="))
        // Swift's Int64 init tolerates a leading '+'; self-generated URLs never contain one, but lock the behavior.
        XCTAssertNotNil(parseMentionPeerId(fromURL: "rg://user?id=+1"))
    }

    func testDateRoundTrip() {
        XCTAssertEqual(dateMarkdownURL(timestamp: 1700000000), "rg://timestamp?t=1700000000")
        XCTAssertEqual(parseDate(fromURL: "rg://timestamp?t=1700000000"), 1700000000)
    }

    func testDateParseRejectsNonMatching() {
        XCTAssertNil(parseDate(fromURL: "rg://user?id=1"))
        XCTAssertNil(parseDate(fromURL: "rg://timestamp?t="))
        XCTAssertNil(parseDate(fromURL: "rg://timestamp?t=notanumber"))
    }

    func testClassifyPrecedence() {
        guard case let .mention(p) = classifyChatLink("rg://user?id=42") else { return XCTFail("expected mention") }
        XCTAssertEqual(p, EnginePeer.Id(42))
        guard case let .date(t) = classifyChatLink("rg://timestamp?t=99") else { return XCTFail("expected date") }
        XCTAssertEqual(t, 99)
    ...
    func testClassifyNearMissesFallToUrl() {
        guard case .url = classifyChatLink("rg://username?id=1") else { return XCTFail("rg://username must be url") }
        guard case .url = classifyChatLink("rg://user?id=abc") else { return XCTFail("non-numeric id must be url") }
        guard case .url = classifyChatLink("rg://timestamp?t=") else { return XCTFail("empty timestamp must be url") }
    }

    func testCodecBoundaryValues() {
        // Zero peer id and a negative timestamp both parse cleanly (no sign/zero special-casing).
        XCTAssertEqual(parseMentionPeerId(fromURL: "rg://user?id=0"), EnginePeer.Id(0))
        XCTAssertEqual(parseDate(fromURL: "rg://timestamp?t=-1"), -1)
    }
}
